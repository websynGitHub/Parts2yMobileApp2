...
namespace YPS.Parts2y.Parts2y_Views
{
    ...
    public partial class CVinInspectQuestionsPage : ContentPage
    {
                ...
                if (checkInternet)
                {
                    ...
                    if (makeitdone == true && selectedTagData.TaskID != 0)
                    {
                        ...
                        if (result?.status == 1)
                        {
                            ...
                            await App.Current.MainPage.DisplayAlert("Done", "This VIN is marked as done.", "Ok");
                            await Vm.TabChange("job");
                            //DependencyService.Get<IToastMessage>().ShortAlert("This VIN is marked as done.");
                        }
                    }
                }
                else
        ...
        private async void NextScanClicked(object sender, EventArgs e)
        {
            ...
            {
                YPSLogger.ReportException(ex, "SfButton_Clicked method -> in CVinInspectQuestionsPage.xaml.cs  " + Settings.userLoginID);
                await service.Handleexception(ex);
            }
        }
        ...
        private async void RadioButton_Clicked(object sender, EventArgs e)
        {
            ...
            {
                YPSLogger.ReportException(ex, "RadioButton_Clicked method -> in CVinInspectQuestionsPage.xaml.cs  " + Settings.userLoginID);
                await service.Handleexception(ex);
            }
        }
    }
