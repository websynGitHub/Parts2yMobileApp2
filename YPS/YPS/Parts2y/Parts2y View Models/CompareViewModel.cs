...
namespace YPS.Parts2y.Parts2y_View_Models
{
    public class CompareViewModel : IBase
    {
            ...
            try
            {
                ...
                else if (pass1 == PermissionStatus.Granted)
                {
                    try
                    {
                        ...
                        {
                            await ZXingScannerPage(scanfor);
                        }
                    }
                    catch (Exception ex1)
                    {
        ...
        private async Task ZXingScannerPage(string scanfor)
        {
            try
            {
                #region Zxing related commented code
            ...
            {
                YPSLogger.ReportException(ex, "ZxingScaneer method -> in CompareViewModel " + YPS.CommonClasses.Settings.userLoginID);
                var trackResult = trackService.Handleexception(ex);
            }
        }
        ...
        public async Task Scanditscan(string scanresult)
        {
            ...
            {
                YPSLogger.ReportException(ex, "Scanditscan method -> in CompareViewModel " + YPS.CommonClasses.Settings.userLoginID);
                var trackResult = trackService.Handleexception(ex);
            }
        }
        ...
        public async void ChangeLabel()
        {
            ...
            catch (Exception ex)
            {
                ...
                YPSLogger.ReportException(ex, "ChangeLabel method -> in CompareViewModel.cs " + Settings.userLoginID);
            }
            finally
            {
                loadindicator = false;
