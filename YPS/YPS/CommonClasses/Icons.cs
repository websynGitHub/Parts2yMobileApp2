namespace YPS.CommonClasses
{
    public static class Icons
    {
        public static readonly string RefreshIc = "\uf021";
        public static readonly string SearchIc = "\uf002";
        public static readonly string SearchPlusIc = "\uf00e";
        public static readonly string CheckBoxIc = "\uf14a";
        public static readonly string CameraIc = "\uf030";
        public static readonly string ChatIc = "\uf086";
        public static readonly string FileAttachIc = "\uf0c6";
        public static readonly string PrintIc = "\uf02b";
        public static readonly string SettingsIc = "\uf013";
        public static readonly string HomeIc = "\uf015";
        public static readonly string OpenFolderIc = "\uf07c";
        public static readonly string InfoIc = "\uf05a";
        public static readonly string MyDeleteIc = "\uf1f8";
        public static readonly string DownloadIc = "\uf019";
        public static readonly string CloseIc = "\uf00d";
        public static readonly string RightDoubleIc = "\uf101";
        public static readonly string QACheckBoxIc = "\uf046";
        public static readonly string UserCountIc = "\uf007";
        public static readonly string MailIc = "\uf0e0";

        public static readonly string owners = "\uf1ad";
        public static readonly string MoreItems = "\uf142";
        public static readonly string users = "\uf007";
        public static readonly string Projects = "\uf03a";
        public static readonly string job = "\uf0b1";
        public static readonly string Entities = "\uf1ea ";
        public static readonly string ShipCtrlReport = "\uf15b";
        public static readonly string DiscrepancyReport = "\uf024";
        public static readonly string SourceDataUpload = "\uf1c0";
        public static readonly string LocationUpload = "\uf041";
        public static readonly string yShip = "\uf21a";
        public static readonly string Configurations = "\uf085";
        public static readonly string MobileAppsIc = "\uf10b";
        public static readonly string Logout = "\uf190";
        public static readonly string Archived = "\uf187";

        public static readonly string minus = "\uf068";
        public static readonly string minusCircle = "\uf056";
        public static readonly string plusCircle = "\uf055";
        public static readonly string chatIcon = "\uf075";
        public static readonly string tickb = "\uf00c";
        public static readonly string tickCircle = "\uf058";
        public static readonly string Flag = "\uf024";
        public static readonly string Exclamation = "\uf12a";
        public static readonly string Calender = "\uf073";
        public static readonly string Notifications = "\uf0f3";

        public static readonly string angledown = "\uf107";
        public static readonly string key = "\uf084";
        public static readonly string undo = "\uf0e2";
        public static readonly string upload = "\uf093";
        public static readonly string edit = "\uf044";
        public static readonly string check = "\uf00c";

        public static readonly string search = "\uf002";
        public static readonly string arrow_circle_right = "\uf0a9";
        public static readonly string CheckCircle = "\uf058";
        public static readonly string circle = "\uf1db";
        public static readonly string righticon = "\uf105";
        public static readonly string Tickicon = "\uf05d";
        public static readonly string Dashboard = "\uf0e4";
        public static readonly string Home = "\uf015";
        public static readonly string QRcode = "\uf029";
        public static readonly string times_circle_o = "\uf05c";
        public static readonly string Cemera = "\uf083";
        public static readonly string Cemera1 = "\uf030";
        public static readonly string User = "\uf007";
        public static readonly string Menubar = "\uf0c9";
        public static readonly string Backnav = "\uf190";

        public static readonly string Grrentick = "\uf00c";

        public static readonly string arrowLeft = "\uf0a8";
        public static readonly string CircleWrong = "\uf057";
        public static readonly string Usericon = "\uf007 ";
        public static readonly string info = "\uf05a";
        public static readonly string bus = "\uf207";

        public static readonly string Task = "\uf0ae";
        public static readonly string Scan = "\uf029";


        public static readonly string reset = "\uf0e2";
        public static readonly string view = "\uf06e";
        public static readonly string back = "\uf060";

        public static readonly string flashOffIC = "\uf185";
        public static readonly string flashOnIC = "\uf0e7";
    }
}
